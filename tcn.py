...
'''
The TCN implementation here is based on the Package keras-tcn (https://github.com/philipperemy/keras-tcn) with special optimizations
...
from tensorflow.keras.layers import Layer, Conv2D, BatchNormalization, LayerNormalization
...
class MyConv1D(Layer):
  def __init__(self,
    ...
    if padding == 'causal':
      self.padding_layer = layers.ZeroPadding2D(((0,0), (2 * dilation_rate, 0)))
    else:
  ...
  def get_config(self):
    ...
    """
    config = super(MyConv1D, self).get_config()
    return config
    ...
    with K.name_scope(self.name):  # name scope used to make sure weights get unique names
      ...
      for k in range(2):
        ...
        with K.name_scope(name):  # name scope used to make sure weights get unique names
          self._add_and_activate_layer(MyConv1D(filters=self.nb_filters,
      ...
      if self.nb_filters != input_shape[-1]:
        ...
        with K.name_scope(name):
          ...
          self.shape_match_conv = MyConv1D(filters=self.nb_filters,
                                           kernel_size=1,
                                           padding='same',
                                           name=name,
                                           kernel_initializer=self.kernel_initializer)
